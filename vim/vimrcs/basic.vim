...
"nmap <F5> :!php %<CR>
nmap <silent> <F5> :call BuildCurFile()<CR>
...
function BuildCurFile()
    ...
    "c需要提取第一行的编译参数
    "如当需要引入第三方库(以mysql为例)时，则在第一行添加://-lmysqlclient -L/usr/local/mysql/include
    ...
    elseif file_ext == "php"
        let file_cmd = '/usr/bin/php'
        let file_args = ' -f '. file_name
    "java先调用javac，再调用java
    ...
        echo "it`s not the right file type"
    endif
